...
ko_test = '음악을 들으면서 여행하는 것은 정말 좋은 경험이에요. 특히, 차 안에서 듣는 음악은 기분을 좋게 만들어 줘요.'

id_num = 1

model = "actor"
checkpoint = "G_9000.pth"
...
with open("./datasets/actor/vocab.txt", "r") as f:
...
stn_tst = get_text(ko_test, hps)
with torch.no_grad():
    x_tst = stn_tst.cuda().unsqueeze(0)
    x_tst_lengths = torch.LongTensor([stn_tst.size(0)]).cuda()
    sid = torch.LongTensor([id_num]).cuda()

    out = net_g.infer(x_tst, x_tst_lengths, sid=None, noise_scale=0.667, noise_scale_w=0.8, length_scale=1)
    audio = out[0][0, 0].data.cpu().float().numpy()
    
    scaled_data = np.int16(audio/np.max(np.abs(audio)) * 32767)

    write("output_actor_vits2.wav", hps.data.sample_rate, scaled_data)
